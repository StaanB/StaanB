# Olá Devs, Stan aqui 😄☕

Dev Full-Stack Pleno-Sênior (React, Next.js, Node, Flutter), hoje com um fluxo de trabalho AI-first — da spec ao deploy. Fã de anime, cultura japonesa e uma boa dose de jogos.

🔗 **Portfólio:** [portfolio-stanley-delta.vercel.app](https://portfolio-stanley-delta.vercel.app)

<img src="./assets/terminal.svg" alt="stanley@dev terminal" />

- 🔭 Hoje trabalho como Dev Front-end Pleno na **Prolog App** (gestão de frotas), atuando também com Flutter e Java
- 🤖 Fluxo AI-first: uso Claude Code, Gemini e Codex da spec ao deploy — inclusive escrevendo as specs eu mesmo (o [portfólio](https://portfolio-stanley-delta.vercel.app) é um case disso)
- 🎓 Faço faculdade de Sistemas Web, hoje de olho no próximo passo: liderança técnica
- 🟢 Empregado atualmente, mas sempre aberto a trocar ideia sobre novos projetos e oportunidades
- 📫 Contato: portfólio, LinkedIn, GitHub ou Gmail, nessa ordem
- ⚡ Curiosidade: vejo animes, amo cultura japonesa e sou fã de jogos

<br/>

![](./profile-3d-contrib/profile-custom-orange.svg)

<div align="center">
<a href="https://skillicons.dev">
  <img src="https://skillicons.dev/icons?i=js,ts,react,nextjs,nodejs,nestjs,ruby,rails,flutter,dart,graphql,docker,git,github,vscode,tailwind,sass,aws,jest,cypress,html,css,vercel" />
</a>
</div>

## Contato
<div>
<a href="https://portfolio-stanley-delta.vercel.app" target="_blank"><img src="https://img.shields.io/badge/-Portf%C3%B3lio-FF6B1A?style=for-the-badge&logo=vercel&logoColor=white" target="_blank"></a>
<a href="https://www.linkedin.com/in/stanley-brenner-dev/" target="_blank"><img src="https://img.shields.io/badge/-LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" target="_blank"></a>
<a href="https://instagram.com/_staanb" target="_blank"><img src="https://img.shields.io/badge/-Instagram-%23E4405F?style=for-the-badge&logo=instagram&logoColor=white" target="_blank"></a>
<a href="mailto:stanleybrenner@gmail.com"><img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" target="_blank"></a>
</div>
